...
| multiclass (32 FDI classes)  | 25     | 0.733          | 0.817       | **0.835**  | 0.499         |

Full metrics (including per-FDI-class breakdown) are in
`outputs/runs/eval/eval_test.json` (binary) and `outputs/runs/eval-3/eval_test.json`
(multiclass); confusion matrices and PR curves are saved alongside them.

**Data quality fix (2026-07-30):** dataset 30's converter (`convert_30.py`)
assumed its YOLO class-index order matched `tooth_seg.taxonomy.FDI_CODES`
(Universal-numbering order), but the dataset's own `data.yaml` actually orders
classes by ascending FDI string - the two orders happen to coincide for
quadrants 2 and 4 but are exactly reversed for quadrants 1 and 3, so every
upper-right (11-18) and lower-left (31-38) annotation from this 1,448-image,
box-only source was trained under its mirror-reversed label. Caught by
auditing each class's spatial position against anatomy (see git history for
the audit) and confirmed by retraining after the fix. Fairly re-evaluating
both models against the **corrected** test labels (the original published
numbers above were measured before the fix and were inflated - the old
model's wrong-but-consistent predictions happened to match the equally-wrong
test labels for the affected classes):

| model                        | precision | recall | mAP50 | mAP50-95 |
| ----------------------------- | --------- | ------ | ----- | -------- |
| old (buggy training data)     | 0.678     | 0.717  | 0.692 | 0.412    |
| new (corrected training data) | **0.733** | **0.817** | **0.835** | **0.499** |

All 16 affected classes improved (+0.09 to +0.27 mAP50-95 each); the 16
unaffected classes moved by at most 0.065 either way, consistent with normal
run-to-run noise - see `git log -- src/tooth_seg/data/converters/convert_30.py`
for the fix. The now-weakest classes are the lower-right and lower-left
central incisors (41, 31) and lower-right lateral incisor (42), not the
wisdom teeth as previously stated here - likely a genuine visual-similarity
issue (all 8 incisors are tightly packed and similar-looking), not a data
volume problem, since annotation counts per class are comparable throughout.
...
why on the held-out test set (numbers below are post-fix):

| confidence               | mean precision | mean recall | mean F1 |
| ------------------------- | -------------- | ----------- | ------- |
| 0.15 (used by the web app) | 0.550          | **0.940**   | 0.692   |
| 0.25 (Gradio default)      | 0.636          | 0.903       | 0.744   |
| 0.41 (F1-optimal)          | 0.746          | 0.804       | **0.772** |

At 0.15, the model misses only ~6% of real teeth, versus ~20% at the
...
set just above the F1-optimal precision/recall break-even).
